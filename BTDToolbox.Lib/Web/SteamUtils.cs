...
namespace BTDToolbox.Lib
{
    ...
    public class SteamUtils
    {
        ...
        private const UInt64 BTDB2AppID = 1276390;
        private const string BTDB2Name = "Bloons TD Battles 2";

        private static Dictionary<UInt64, string> steamGames = new Dictionary<UInt64, string>
        {{BTD5AppID, BTD5Name}, {BTDBAppID, BTDBName}, {BTDB2AppID, BTDB2Name}, {BMCAppID, BMCName}, {BTD6AppID, BTD6Name},
        ...
        {{GameType.BloonsTD5, BTD5AppID}, {GameType.BloonsTDB, BTDBAppID}, {GameType.BloonsTDB2, BTDB2AppID}, {GameType.BloonsMC, BMCAppID}, {GameType.BloonsTD6, BTD6AppID},
            {GameType.BloonsAT, BTDATAppID}};
        ...
        public static string GetGameDir(UInt64 appid, string gameName)
        {
            ...
            if (steamDir == null)
            {
                Logger.WriteLine("Failed to find steam in registry!");
                return null;
            }

            if (!IsGameInstalled(appid))
            {
                Logger.WriteLine(gameName + " is not installed!");
                return null;
            }
            ...
            Logger.WriteLine(gameName + "'s Directory not found!");
            return null;
        }
        ...
        private void StartValidator(GameType gameType)
        {
            ...
            {
                Logger.WriteLine("Failed to validate " + gameType + ". Failed to find game directory through steam.");
                return;
            }

            if (IsGameRunning(gameType))
            {
                Logger.WriteLine(gameType + " is currently running. Please close and try again.");
                return;
            }    
