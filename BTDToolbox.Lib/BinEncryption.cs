...
public static class BinEncryption
{
    ...
    /// <param name="saveDecryptedText">Whether or not the file should be overwritten with the decrypted text.</param>
    /// <returns>The decrypted text from this file.</returns>
    public static string DecryptFile(ZipFile zipFile, ZipEntry entry, bool saveDecryptedText = false)
    {
        if (entry == null)
            return null;

        // This entry doesn't have a BIN2.0 header. Assuming it's already been decrypted.
        if (!IsEncrypted(zipFile, entry))
        {
        ...
        Span<byte> textBytes = new Span<byte>(new byte[entry.Size]);
    ...
    /// <returns>True if the file was successfully encrypted. False if it failed to encrypt or was already encrypted.</returns>
    public static bool EncryptFile(ZipFile zipFile, ZipEntry entry)
    {
        if (IsEncrypted(zipFile, entry))
            return false;
    ...
    public static bool IsEncrypted(ZipFile zipFile, ZipEntry entry)
    {
        if (zipFile == null || entry == null)
            throw new Exception("Couldn't encrypt Entry because it was null");

        return HasBinHeader(zipFile, entry);
    }
    ...
    private static bool HasBinHeader(ZipFile zipFile, ZipEntry entry)
    {
        var stream = zipFile.GetInputStream(entry);
        if (entry.Size < header.Length)
        {
            stream.Close();
            return false;
        }
